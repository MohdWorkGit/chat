  ...
  template: `
    <div class="p-6">
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-lg font-semibold text-gray-900">Label Reports</h2>
      ...
      @if (loading) {
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
              @for (label of reportData; track label.labelId) {
                <tr class="hover:bg-gray-50">
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-2">
                      <span
                        class="h-3 w-3 rounded-full"
                        [style.background-color]="label.labelColor || '#6B7280'"
                      ></span>
                      <span class="text-sm text-gray-900">{{ label.labelName }}</span>
                    </div>
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ label.totalConversations }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ label.resolvedConversations }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ label.avgFirstResponseTime }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ label.avgResolutionTime }}</td>
                </tr>
              }
            </tbody>
          </table>
        ...
        <div class="text-center py-12">
          <p class="text-sm text-gray-500">Select a date range to view label reports.</p>
        </div>
      }
    </div>
  `,
