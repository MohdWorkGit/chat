...
@Component({
  selector: 'app-inbox-report',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  template: `
    <div class="p-6">
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-lg font-semibold text-gray-900">Inbox Reports</h2>
      ...
      @if (loading) {
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Inbox</th>
            ...
            <tbody class="divide-y divide-gray-200">
              @for (inbox of reportData; track inbox.inboxId) {
                <tr class="hover:bg-gray-50">
                  <td class="px-6 py-4 text-sm text-gray-900">{{ inbox.inboxName }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ inbox.totalConversations }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ inbox.resolvedConversations }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ inbox.avgFirstResponseTime }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ inbox.avgResolutionTime }}</td>
                </tr>
              }
            </tbody>
          </table>
        ...
        <div class="text-center py-12">
          <p class="text-sm text-gray-500">Select a date range to view inbox reports.</p>
        </div>
      }
    </div>
  `,
...
export class InboxReportComponent implements OnInit {
