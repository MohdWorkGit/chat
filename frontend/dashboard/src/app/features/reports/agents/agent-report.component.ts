    ...
    <div class="p-6">
      ...
      @if (loading) {
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
              @for (agent of reportData; track agent.agentId) {
                <tr class="hover:bg-gray-50">
                  <td class="px-6 py-4 text-sm text-gray-900">{{ agent.agentName }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ agent.totalConversations }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ agent.resolvedConversations }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ agent.avgFirstResponseTime }}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">{{ agent.avgResolutionTime }}</td>
                </tr>
              }
            </tbody>
          </table>
        ...
        <div class="text-center py-12">
          <p class="text-sm text-gray-500">Select a date range to view agent performance reports.</p>
        </div>
      }
    </div>
  `,
...
export class AgentReportComponent implements OnInit {
  ...
  loadReport(): void {
    this.loading = true;
    const params: Record<string, string> = { type: 'agent' };
